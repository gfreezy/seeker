...
- Allow the anonymous default proxy group used by bare `PROXY`/`PROBE` rules to be persisted as `name: ""`; persisted default groups keep tracking all current servers and can use `type: select` with `default_selected`.
