...
use crate::udp_io::{encrypt_payload, decrypt_payload};
...
impl SSClient {
    ...
    pub async fn handle_udp_connection(&self, tun_socket: TunUdpSocket, addr: Address) -> Result<()> {
        let ssserver = get_remote_ssserver_addr(
            &*self.resolver,
            self.srv_cfg.clone(),
            (&self.dns_server.0, self.dns_server.1),
...
//        };
        ...
        loop {
            ...
            let (recv_from_tun_size, local_src) = tun_socket.recv_from(&mut buf[addr.serialized_len()..]).await?;
            debug!("recv {} bytes from tun {}", recv_from_tun_size, &local_src);
            let encrypt_size = encrypt_payload(cipher_type, &key, &buf[..addr.serialized_len()+recv_from_tun_size], &mut encrypt_buf)?;
            debug!("encrypt {} bytes with {} to {} bytes", addr.serialized_len()+recv_from_tun_size, cipher_type, encrypt_size);
                ...
                None => {
                    ...
                    let _handle: JoinHandle<Result<()>> = task::spawn(async move {
                        ...
                        loop {
                            ...
                            let decrypt_size = decrypt_payload(cipher_type, &key_cloned, &recv_buf[..recv_from_ss_size], &mut decrypt_buf)?;
                            debug!("decrypt {} bytes with {} to {} bytes", recv_from_ss_size, cipher_type, decrypt_size);
                            let addr = Address::read_from(&mut decrypt_buf.as_ref())?;
                            let send_local_size = cloned_socket.send_to(&decrypt_buf[addr.serialized_len()..decrypt_size], &local_src).await?;
            ...
            let send_ss_size = udp_socket.send_to(&encrypt_buf[..encrypt_size], ssserver).await?;
