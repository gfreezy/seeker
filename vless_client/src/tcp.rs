...
impl VlessTcpStream {
    ...
    ///
    /// Feeds data through `read_tls()` which uses the session's internal deframer.
    /// Tracks bytes unconsumed by `read_tls()` in `tcp_read_buf` — these are saved
    /// for later and become raw data on Direct mode transition.
    fn poll_read_tls_plaintext(&mut self, cx: &mut Context<'_>) -> Poll<Result<Vec<u8>>> {
        ...
        // Feed buffered TCP data first (unconsumed from previous read)
        if !self.tcp_read_buf.is_empty() {
            let data = std::mem::take(&mut self.tcp_read_buf);
            let mut cursor = std::io::Cursor::new(data.as_slice());
            tls.read_tls(&mut cursor)
                .map_err(|e| Error::new(ErrorKind::InvalidData, format!("TLS read_tls: {e}")))?;
            let consumed = cursor.position() as usize;
            if consumed < data.len() {
                self.tcp_read_buf = data[consumed..].to_vec();
            }
            let state = tls.process_new_packets().map_err(|e| {
                Error::new(
                    ErrorKind::InvalidData,
        ...
        // Feed to TLS session, track unconsumed bytes
        let mut cursor = std::io::Cursor::new(&tcp_buf[..filled]);
        tls.read_tls(&mut cursor)
            .map_err(|e| Error::new(ErrorKind::InvalidData, format!("TLS read_tls: {e}")))?;
        let consumed = cursor.position() as usize;
        if consumed < filled {
            // Save bytes the session's deframer couldn't accept (buffer full)
            self.tcp_read_buf.extend_from_slice(&tcp_buf[consumed..filled]);
        }
        ...
        // TLS record not complete yet, need more data
