...
impl ProxyClient {
    pub async fn new(config: Config, uid: Option<u16>) -> Self {
        let session_manager =
    ...
    async fn choose_proxy_tcp_stream(&self, remote_addr: &Address) -> Result<ProxyTcpStream> {
        ...
        Ok(ProxyTcpStream::Direct(TcpStream::connect(addr).await?))
    }

    async fn choose_proxy_udp_socket(&self, addr: SocketAddr) -> Result<ProxyUdpSocket> {
        if !self.probe_connectivity(addr).await {
            if let Some(socks5_config) = &self.config.socks5_server {
                let server = self.resolve(&socks5_config.addr).await?;
    ...
    async fn run_tcp_relay_server(&self) -> Result<()> {
        ...
        while let Some(Ok(conn)) = incoming.next().await {
            ...
            match self.choose_proxy_tcp_stream(&host).await {
                Ok(remote_conn) => {
                    spawn(
                        async move {
                            trace!("tunneling");
    ...
    async fn new_udp_socket(&self, port: u16) -> Result<(ProxyUdpSocket, SocketAddr)> {
        ...
        let addr = match self
            ...
            .await
        {
            Some(ip_addr) => SocketAddr::new(ip_addr, real_dest.port()),
            None => real_dest,
        };
        let socket = self.choose_proxy_udp_socket(addr).await?;
        self.udp_manager
            .write()
            .insert(port, (socket.clone(), addr));
        Ok((socket, addr))
    }
    ...
    async fn run_udp_relay_server(&self) -> Result<()> {
        ...
        loop {
            ...
            let (socket, dest_addr) =
                match self.get_udp_socket_and_dest_addr(peer_addr.port()).await {
                    None => {
                        let (socket, dest_addr) = self.new_udp_socket(peer_addr.port()).await?;
                        ...
                        spawn(async move {
                            let _: Result<()> = async {
                                ...
                                loop {
                                    let (recv_size, _peer) =
                                        timeout(recv_timeout, socket_clone.recv_from(&mut buf))
                                            .await?;
