...
使用 Tun 实现透明代理（支持 Mac & Linux 系统），支持 TCP、UDP、ICMP（ping）。支持 Shadowsocks、SOCKS5、HTTP/HTTPS、Hysteria2、Trojan、VMess 和 VLESS 代理协议。
...
* `seeker` 支持 socks5 代理、http 代理、shadowsocks 代理、Hysteria2、Trojan、VMess 和 VLESS 代理。
...
proxies:
  - name: "[SS] Hong Kong-20"
    type: ss                             # 支持: ss, socks5, http, https, hy2, trojan, vmess, vless
...
在配置文件中使用 `remote_config_urls` 字段，从远程 URL 加载 Shadowsocks 服务器列表。支持**标准 SS 订阅格式**，兼容大多数 SS 服务商提供的订阅链接。
...
`seeker` 的 `proxy_groups` 支持 `select` 和 `url_test` 两种选择策略：
...
* `select`: 固定使用 `default_selected` 指定的代理，不会因测速或连续连接失败自动切换。如果 `default_selected` 未设置或指定的代理不在组内，则使用 `proxies` 第一项。
* `url_test`: 根据延迟和成功率自动选择最佳代理。为保持兼容，未配置 `type` 的代理组默认使用 `url_test`。
* 默认路由组通过最后一条 `MATCH,PROXY(group-name)` 规则指定。
* 裸 `PROXY`/`PROBE` 使用名称为空的默认组。需要固定该组时，可在 `proxy_groups` 中配置 `name: ""`、`type: select` 和 `default_selected`；该组始终包含全部当前服务器。

=== 服务器切换优化

`seeker` 在 `url_test` 代理组中自动选择最佳服务器时，采用以下优化策略以减少频繁切换：
...
`seeker` 支持通过 HTTP API 查询代理服务器的性能数据。在配置文件中设置 `api_addr` 即可启动 API 服务器：

[source,yaml]
----
api_addr: "127.0.0.1:7890"
----

启动后可以通过 `GET /api/stats` 获取所有代理组的性能数据：

[source,bash]
----
curl http://127.0.0.1:7890/api/stats
----
...
NOTE: 密码学算法由 RustCrypto 套件（`aes`、`cfb-mode`、`cfb8`、`ctr`、`camellia`、`aes-gcm`、`chacha20poly1305`）和 `ring`、`libsodium` 提供。原 OpenSSL 后端已完全移除，`aes-*-cfb1` 这类 1-bit 反馈模式没有纯 Rust 实现，因此不再支持（属于已废弃的 Shadowsocks 遗留算法）。
...
`seeker` 支持 Trojan 协议，基于 TLS 的代理协议。
...
[source]
----
trojan://password@host:port?peer=sni&allowInsecure=1#name
----

=== VMess 配置

`seeker` 支持 VMess 协议（V2Ray 的加密传输协议），支持 AEAD 加密。
...
servers:
  - name: server-vmess
    addr: example.com:443
    protocol: Vmess                # 也可以使用 vmess
    password: uuid                 # VMess UUID
    sni: example.com               # 可选，默认使用 addr 的域名
    # insecure: false              # 可选，跳过 TLS 证书验证
----
...
`seeker` 支持 VLESS 协议（V2Ray 的轻量传输协议），支持 XTLS-Vision flow。
...
servers:
  - name: server-vless
    addr: example.com:443
    protocol: Vless                # 也可以使用 vless
    password: uuid                 # VLESS UUID
    sni: example.com               # 可选，默认使用 addr 的域名
    # flow: xtls-rprx-vision       # 可选，启用 XTLS-Vision
    # insecure: false              # 可选，跳过 TLS 证书验证
----
...
[source]
----
vless://uuid@host:port?security=tls&sni=xxx&flow=xtls-rprx-vision&allowInsecure=1#name
----

== ⚠️使用 Socks5 或 http 代理服务器
使用 socks5 代理的时候，需要将所有直连的域名设置在配置文件里面，如果使用 ss、vmess、vless、trojan 之类的，需要将代理服务器的域名也加入配置文件设为直连。否则有可能会导致死循环，没法正常使用。
...
NOTE: `seeker` 使用纯 Rust 的密码学实现（RustCrypto 套件）和 `rustls` 作为 TLS 栈，不依赖 OpenSSL，也不再需要 `OPENSSL_STATIC=yes` 环境变量。
...
. 当 TUN 设备收到 ICMP Echo Request 时，`seeker` 会拦截该请求
...
. `seeker` 从 TUN 接受到 IP 包后，会在内部组装成 TCP/UDP/ICMP 数据
