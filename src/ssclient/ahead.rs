    ...
    W: AsyncWrite,
{
    ...
    fn encrypt<B: BufMut>(&mut self, data: &[u8], buf: &mut B) -> io::Result<()> {
        ...
        self.cipher.encrypt(&data_len_buf, unsafe {
            &mut buf.bytes_mut()[..output_length_size]
        });
        self.cipher.encrypt(data, unsafe {
            &mut buf.bytes_mut()[output_length_size..output_length]
        });
